...
@router.post("/workspace/{workspace_id}", response_model=ProjectResponse, status_code=status.HTTP_201_CREATED)
def create_project(
    workspace_id: uuid.UUID,
    ...
    if not workspace:
        raise HTTPException(
            status_code=status.HTTP_444_NOT_FOUND, # Custom status code or 404
            detail="Workspace not found"
        )
    if not workspace:
        raise HTTPException(
